...
export default function powerlineFooter(pi: ExtensionAPI) {
  ...
  async function showSelectOverlay(
    ...
    const custom = ctx.ui.custom as <T>(
      factory: (tui: any, theme: Theme, keybindings: any, done: (result: T) => void) => void,
      options?: unknown,
    ) => Promise<T>;
  ...
  pi.on("session_start", async (event, ctx) => {
    ...
    const ctxWithThinking = ctx as unknown as { getThinkingLevel?: () => string | null };
    getThinkingLevelFn = typeof ctxWithThinking.getThinkingLevel === "function"
      ? () => ctxWithThinking.getThinkingLevel?.() ?? null
      : null;
  ...
  pi.registerShortcut("alt+s" as KeyId, {
