...
`ctx_execute` and `ctx_execute_file` run in a bounded child process, not an OS
sandbox. They can read the project and user files and can use the network.
Credential-like environment variables are removed by this vendored build, but
do not place secrets in code, arguments, files, or fetched URLs.
...
| Playwright snapshot (one-shot) | `browser_snapshot(filename)` → `ctx_execute_file(path)` | Save to file, extract in a child process |
| Playwright console/network | `browser_*(filename)` → `ctx_execute_file(path)` | Save to file, analyze in a child process |
...
**When a task involves Playwright snapshots, screenshots, or page inspection, ALWAYS route through file → child process.**
...
**The key insight**: `browser_snapshot` has a `filename` parameter that saves to file instead of returning to context. `ctx_index` has a `path` parameter that reads files server-side. `ctx_execute_file` processes files in a child process. **None of these touch context.**
...
Step 2: ctx_execute_file(path: "/tmp/playwright-snapshot.md", language: "javascript", code: "
        ...
        ")
        → processes in a child process, returns ~200B summary
```
...
- Expecting `ctx_stats` to reset or wipe anything → `ctx_stats` is read-only. Destructive purge is intentionally not exposed by this vendored build.
