...
import { existsSync, unlinkSync, readdirSync, readFileSync, writeFileSync, writeSync, renameSync, rmSync, mkdirSync, cpSync, statSync, lstatSync, realpathSync } from "node:fs";
...
// Local vendor policy: upgrades remain repository-managed, browser launch is
// never implicit, and destructive deletion stays outside the model tool set.
const VENDOR_DISABLED_TOOLS = new Set([
  "ctx_upgrade",
  "ctx_insight",
  "ctx_purge",
]);
...
(server as unknown as { registerTool: (...args: unknown[]) => unknown }).registerTool = (...args: unknown[]) => {
  ...
  if (VENDOR_DISABLED_TOOLS.has(name)) return undefined;
...
function getStore(): ContentStore {
  if (!_store) {
    ...
    // Local vendor policy: persistent indexes are never deleted implicitly.
    // Retention and cleanup are repository/user-managed operations.
  }
...
// Local vendor builds do not contact npm for update discovery. Updates are
// reviewed and committed in this repository instead.
...
function trackResponse(toolName: string, response: ToolResult): ToolResult {
  ...
  noteMcpActivity();
  // Vendored builds never mutate another host's plugin cache. The upstream
  // healCacheMidSession() routine is intentionally not invoked here.
