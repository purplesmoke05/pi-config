...
import {
  safeChildProcessEnvironment,
  isSensitiveEnvironmentName,
} from "./credential-env.js";
export { isSensitiveEnvironmentName } from "./credential-env.js";
...
export class PolyglotExecutor {
  ...
  async #compileAndRun(
    ...
    try {
      execFileSync("rustc", [srcPath, "-o", binPath], {
        cwd,
        env: this.#buildSafeEnv(dirname(srcPath)),
  ...
  #buildSafeEnv(tmpDir: string): Record<string, string> {
    ...
    // One shared boundary protects the bridge, probes, compilers, and executed
    // code from both credential leakage and environment-based code injection.
    const env = safeChildProcessEnvironment();
