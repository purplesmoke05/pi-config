...
/**
 * Rebind the default coding toolset to a worktree cwd without widening an
 * explicitly injected restricted toolset (for example, web-only research).
 */
export function toolsForRunCwd(
  baseTools: ToolDefinition[],
  usesDefaultCodingTools: boolean,
  baseCwd: string,
  runCwd: string,
): ToolDefinition[] {
  if (runCwd === baseCwd || !usesDefaultCodingTools) return baseTools;
  return createCodingTools(runCwd);
}

export class WorkflowAgent {
  private readonly cwd: string;
  private readonly baseTools: ToolDefinition[];
  private readonly usesDefaultCodingTools: boolean;
  ...
  constructor(options: WorkflowAgentOptions = {}) {
    ...
    this.usesDefaultCodingTools = options.tools === undefined;
  ...
  async run<TSchemaDef extends TSchema | undefined = undefined>(
    ...
    const baseTools = toolsForRunCwd(this.baseTools, this.usesDefaultCodingTools, this.cwd, runCwd);
