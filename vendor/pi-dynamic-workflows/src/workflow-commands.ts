...
import { type CustomWorkflowApproval, requireCustomWorkflowApproval } from "./workflow-trust.js";
  ...
  pi.registerCommand("workflows", {
    ...
    async handler(args: string, ctx: ExtensionCommandContext) {
      ...
      switch (sub) {
        ...
        case "resume": {
          if (!id) return ctx.ui.notify(USAGE, "warning");
          const persisted = manager.listRuns().find((run) => run.runId === id);
          let approval: CustomWorkflowApproval | undefined;
          if (persisted?.script && persisted.source !== "builtin") {
            try {
              approval = await requireCustomWorkflowApproval(
                { source: "resume", label: id, script: persisted.script },
                ctx,
              );
            } catch (error) {
          ...
          let ok: boolean;
          try {
            ok = await manager.resume(id, { approval });
          } catch (error) {
            ctx.ui.notify(error instanceof Error ? error.message : String(error), "warning");
            return;
          }
