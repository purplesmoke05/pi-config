          ...
          switch (action.type) {
            ...
            case "restart": {
              ...
              const restart = async () => {
                if (run.source === "builtin") {
                  const builtin = run.builtinName ? findBuiltinWorkflow(run.builtinName) : undefined;
                  if (!builtin) {
                    throw new Error("persisted built-in provenance is missing or unknown");
                  }
                  const invocation = builtin.resolve(opts.cwd ?? process.cwd(), run.args);
                  return manager.startInBackground(invocation.script, invocation.args, {
                    source: "builtin",
                ...
                return manager.startInBackground(run.script, run.args, {
                  source: "custom",
                  approval,
                });
              };
            ...
            case "save": {
              ...
              if (!run?.script) {
                ...
                registerSavedWorkflow(pi, saved, manager, () => storage.list().some((w) => w.name === saved.name));
