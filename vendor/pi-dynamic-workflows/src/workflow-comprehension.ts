...
export const COMPREHENSION_SCENARIOS: readonly ComprehensionScenario[] = [
  ...
  },
  {
    ...
    prompt: `${ENVELOPE}\nEdit the workflow below. The audited host rejects in-script workflow(...) child calls because child code is outside the approved script. Declare and enter a phase with a positive phase budget, ask one preparation agent, replace the forbidden child call with direct sequential agent() calls for alpha and beta, and return both child results. Consult any installed skill that applies.\n\nCurrent workflow:\nexport const meta = { name: "child_jobs", description: "broken" }\nreturn await workflow("child-workflow", { id: "alpha" })`,
  },
  {
    id: "full-review",
    suite: ComprehensionSuite.FULL,
...
function assertScenario(
  ...
  if (scenario.id === "full-edit") {
    const alphaCall = findTaskCall(runtime.calls, "alpha");
    const betaCall = findTaskCall(runtime.calls, "beta");
    const preparationCalls = alphaCall ? runtime.calls.filter(({ index }) => index < alphaCall.index) : [];
    const preparation = preparationCalls[0];
    assertions.push({
      name: "preparation:executed",
      passed: preparationCalls.length === 1,
      details: "edit scenario must execute exactly one preparation agent before inline child jobs",
    });
    assertions.push({
      name: "child-jobs:inline",
      passed:
        alphaCall !== undefined && betaCall !== undefined && !runtime.events.some((event) => event.type === "workflow"),
      details: "alpha and beta must execute as direct agents without any nested workflow event",
    });
    assertions.push({
      name: "child-jobs:sequential",
      passed:
        alphaCall?.completedIndex !== null &&
        alphaCall?.completedIndex !== undefined &&
        betaCall !== undefined &&
        alphaCall.completedIndex < betaCall.index,
      details: "inline child agents must execute sequentially in alpha then beta order",
    });
    assertions.push({
      name: "child-jobs:returned-results",
      passed:
        alphaCall !== undefined &&
        betaCall !== undefined &&
        containsValue(runtime.result, alphaCall.result) &&
        containsValue(runtime.result, betaCall.result),
      details: "edit scenario must return data from both inline child agents",
    });
    assertions.push({
      name: "phase:entered",
      passed:
