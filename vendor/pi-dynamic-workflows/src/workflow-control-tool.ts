...
const workflowControlSchema = Type.Object(
  {
    action: Type.Union(
      ...
      {
        description:
          "list = all runs (no runId); status/pause/resume/stop act on one run and require runId. Resume re-validates curated built-in provenance; custom and legacy code requires fresh complete-script interactive approval and fails closed headlessly.",
      },
    ),
    runId: Type.Optional(
      Type.String({
...
export function createWorkflowControlTool(
  ...
  return defineTool({
    ...
    description:
      "List and inspect workflow runs, or pause, resume, and stop them without asking the user to run slash commands. Resume re-validates curated built-ins; custom and legacy scripts require fresh complete-script interactive approval.",
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      try {
        switch (params.action) {
          ...
          case "resume":
            if (run.source === "builtin") {
              if (!(await manager.resume(run.runId))) return invalidTransition("resume", run);
            } else {
              const approval = await requireCustomWorkflowApproval(
                { source: "resume", label: run.runId, script: run.script },
                ctx,
              );
              if (!(await manager.resume(run.runId, { approval }))) return invalidTransition("resume", run);
            }
