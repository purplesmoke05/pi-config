...
import {
  type AuthorizedBuiltinWorkflow,
  type BuiltinWorkflowAuthorization,
  type BuiltinWorkflowInvocation,
  consumeBuiltinWorkflowAuthorization,
  findBuiltinWorkflow,
...
import {
  ...
  CustomWorkflowApprovalError,
  consumeCustomWorkflowApproval,
  type WorkflowExecutionSource,
} from "./workflow-trust.js";
...
export interface ManagedRun {
  ...
  script: string;
  /** Audited built-in or human-approved custom code. Missing persisted values fail closed as custom. */
  source: WorkflowExecutionSource;
  /** Registry identity required to re-derive and verify a built-in on cold resume. */
  builtinName?: string;
  args?: unknown;
...
export interface ExecOptions {
  /** Trust provenance for this script. Omitted callers are treated as custom. */
  source?: WorkflowExecutionSource;
  /** Human approval for this exact custom script, produced by the UI trust gate. */
  approval?: CustomWorkflowApproval;
  /** One-shot registry provenance for one of the five curated built-ins. */
  builtinAuthorization?: BuiltinWorkflowAuthorization;
  /**
...
/**
 * Generation-bound capability operations used at the execution boundary.
 *
 * Pi evaluates every extension reload through a fresh Jiti module graph. The
 * approval registries in workflow-trust.ts and builtin-workflows.ts are
 * deliberately module-private WeakMaps, so a live manager handed across a
 * reload must consume capabilities through the NEW generation that issued
 * them. Keeping these three operations together makes that dependency
 * explicit and lets reconfigureAfterReload() replace it atomically.
 */
export interface WorkflowExecutionTrust {
  consumeCustom(approval: CustomWorkflowApproval | undefined, script: string): boolean;
  consumeBuiltin(
    authorization: BuiltinWorkflowAuthorization | undefined,
    script: string,
  ): AuthorizedBuiltinWorkflow | undefined;
  resolveBuiltin(name: string, cwd: string, args: unknown): BuiltinWorkflowInvocation | undefined;
}

/** Capability issuer/consumer set for this module generation. */
export const WORKFLOW_EXECUTION_TRUST: WorkflowExecutionTrust = Object.freeze({
  consumeCustom: consumeCustomWorkflowApproval,
  consumeBuiltin: consumeBuiltinWorkflowAuthorization,
  resolveBuiltin(name: string, cwd: string, args: unknown) {
    return findBuiltinWorkflow(name)?.resolve(cwd, args);
  },
});
...
export interface WorkflowManagerOptions {
  ...
  /** Current Pi project-trust decision. Repository-owned agent definitions are
   * excluded unless this callback returns true at run start. */
   ...
   * Called lazily per execution (including on resume). An unknown tag fails
   * explicitly instead of widening to the default coding tools.
   */
  ...
  /** Enforce exact-script approval and disable unattended resume for custom code. */
  enforceCustomWorkflowApproval?: boolean;
  /**
   * Capability issuer/consumer set for the extension module generation that is
   * currently presenting approval UI and resolving curated built-ins. Host
   * wiring refreshes this on /reload; ordinary embedders should omit it.
   */
  executionTrust?: WorkflowExecutionTrust;
  /**
...
export type WorkflowManagerReloadOptions = Pick<
  ...
  | "isProjectTrusted"
  | "enforceCustomWorkflowApproval"
  | "executionTrust"
>;

export function resolveWorkflowTools(
  explicitTools: ToolDefinition[] | undefined,
  toolset: string | undefined,
  toolsets: Record<string, () => ToolDefinition[]> | undefined,
): ToolDefinition[] | undefined {
  if (explicitTools) return explicitTools;
  if (!toolset) return undefined;
  const factory = toolsets?.[toolset];
  if (!factory) {
    throw new Error(`Workflow toolset "${toolset}" is not registered.`);
  }
  return factory();
}
...
export class WorkflowManager extends EventEmitter {
  ...
  private enforceCustomWorkflowApproval: boolean;
  private executionTrust: WorkflowExecutionTrust;
  ...
  constructor(options: WorkflowManagerOptions = {}) {
    ...
    this.enforceCustomWorkflowApproval = options.enforceCustomWorkflowApproval ?? false;
    this.executionTrust = options.executionTrust ?? WORKFLOW_EXECUTION_TRUST;
  ...
  reconfigureAfterReload(options: WorkflowManagerReloadOptions): void {
    ...
    this.enforceCustomWorkflowApproval = options.enforceCustomWorkflowApproval ?? false;
    this.isProjectTrusted = options.isProjectTrusted ?? (() => false);
    if (options.executionTrust) this.executionTrust = options.executionTrust;
  }
  ...
  private authorizeExecution(script: string, args: unknown, exec: ExecOptions): AuthorizedExecution {
    const requestedSource = exec.source ?? "custom";
    if (requestedSource === "builtin") {
      const builtin = this.executionTrust.consumeBuiltin(exec.builtinAuthorization, script);
      if (builtin) {
        return {
          source: "builtin",
    ...
    // Compatibility embedders may disable the human approval gate, but a
    // caller-provided source label is never built-in provenance. Without a
    // genuine registry token, classify the execution as custom so persistence,
    // restart, and auto-resume policy cannot inherit forged built-in authority.
    if (!this.enforceCustomWorkflowApproval) {
      return {
        source: "custom",
        args,
        tools: exec.tools,
    ...
    if (this.executionTrust.consumeCustom(exec.approval, script)) {
      return {
        source: "custom",
        args,
        tools: exec.tools,
    ...
    throw new CustomWorkflowApprovalError(
      "Workflow execution requires either one-shot provenance from the curated built-in registry or a fresh interactive approval for this exact custom script; unattended/headless execution is disabled.",
    );
  }
  ...
  startInBackground(
    ...
    const authorized = this.authorizeExecution(script, args, exec);
    const { source } = authorized;
    args = authorized.args;
    exec = { ...exec, tools: authorized.tools, toolset: authorized.toolset };
    ...
    const managed: ManagedRun = {
      ...
      script,
      source,
      builtinName: authorized.builtinName,
      args,
      journal: [],
      background: true,
      lease,
      autoResume: this.enforceCustomWorkflowApproval && source === "custom" ? false : exec.autoResume,
    ...
    try {
      ...
      this.persistence.save({
        ...
        script,
        source,
        builtinName: managed.builtinName,
        args,
  ...
  async runSync(script: string, args?: unknown, exec: ExecOptions = {}): Promise<WorkflowRunResult> {
    ...
    managed.builtinName = authorized.builtinName;
    ...
    managed.lease = lease;
    managed.autoResume = this.enforceCustomWorkflowApproval && source === "custom" ? false : exec.autoResume;
  ...
  private async executeRun(
    ...
    try {
      const result = await runWorkflow(script, {
        ...
        includeProjectAgentDefinitions: this.isProjectTrusted(),
        ...
        tools: resolveWorkflowTools(tools, managed.toolset, this.toolsets),
        excludeTools: this.excludeSubagentTools,
        confirm,
        loadSavedWorkflow: this.loadSavedWorkflow,
        allowNestedWorkflows: !this.enforceCustomWorkflowApproval,
        resumeJournal,
  ...
  async resume(
    runId: string,
    opts?: { script?: string; args?: unknown; approval?: CustomWorkflowApproval },
  ): Promise<boolean> {
    ...
    let args = opts?.args !== undefined ? opts.args : persisted.args;
    let builtinAuthorization: BuiltinWorkflowAuthorization | undefined;
    if (opts?.script === undefined && persisted.source === "builtin" && persisted.builtinName) {
      try {
        builtinAuthorization = this.executionTrust.resolveBuiltin(persisted.builtinName, this.cwd, args)?.authorization;
      } catch {
        // Invalid or tampered persisted built-in inputs fail into the custom
        // approval path below; unattended resume is never granted by metadata.
      }
    }
    const authorized = this.authorizeExecution(script, args, {
      source: opts?.script !== undefined ? "custom" : persisted.source === "builtin" ? "builtin" : "custom",
    ...
    const { source } = authorized;
    args = authorized.args;
    // Persisted display metadata is owner-writable state, not execution
    // provenance. Re-derive it from the exact script that just crossed the
    // authorization boundary so a cold resume cannot spoof its name/phases in
    // the navigator while executing different audited code.
    const parsed = parseWorkflowScript(script);
    const lease = this.persistence.acquireRunLease(runId);
    ...
    const managed: ManagedRun = {
      ...
      autoResume: this.enforceCustomWorkflowApproval && source === "custom" ? false : persisted.autoResume,
