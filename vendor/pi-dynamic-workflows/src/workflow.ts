...
export interface WorkflowRunOptions extends WorkflowAgentOptions {
  ...
  /**
   * Whether repository-owned `.pi/agents` definitions may be loaded. Defaults
   * false; the Pi extension enables it only while project trust is active.
   */
  includeProjectAgentDefinitions?: boolean;
  concurrency?: number;
  ...
  /** Disable workflow() nesting when the host cannot approve every child script exactly. */
  allowNestedWorkflows?: boolean;
  /**
...
export async function runWorkflow<T = unknown>(
  ...
  const agentRegistry =
    ...
    loadAgentRegistry(baseCwd, {
      includeProject: options.includeProjectAgentDefinitions === true,
    });
  ...
  const agentImpl = async (prompt: string, agentOptions: AgentOptions = {}) => {
    ...
    return limiter(async () => {
      ...
      // Emit a start only after mandatory isolation has been established. A
      // failed worktree setup never leaves a phantom "running" agent in UI.
      options.onAgentStart?.({ id: deltaKey, label, phase: assignedPhase, prompt, model: displayModel });
  ...
  const workflowFn = async (nameOrScript: string, childArgs?: unknown) => {
    throwIfAborted();
    if (options.allowNestedWorkflows === false) {
      throw new WorkflowError(
        "Nested workflow() execution is disabled because child code is not covered by the parent script approval. " +
          "Inline the child logic directly in the approved parent script.",
