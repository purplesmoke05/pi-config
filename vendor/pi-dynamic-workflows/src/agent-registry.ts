 ...
 * Frontmatter binds the subagent's tools, model, and a body prompt; explicitly
 * trusted project definitions win over both user-level locations on a name collision. This mirrors
...
function readDefsFromDir(dir: string, source: "project" | "user", trustedRoot?: string): AgentDefinition[] {
  ...
  for (const file of files.sort()) {
    try {
      ...
      if (realDirectory) {
        ...
        if (!isWithin(realDirectory, realpathSync(path))) continue;
      }
      const def = parseAgentDefinition(readFileSync(path, "utf-8"), source, file);
...
/**
 * Load the agent registry once for a run. When `includeProject` is explicitly
 * true, scans the project dir first, then the
...
export function loadAgentRegistry(
  ...
  if (opts?.includeProject === true) {
    for (const def of readDefsFromDir(projectDir, "project", cwd)) {
