...
export class UsageLimitScheduler {
  ...
  private handlePaused(event: { runId?: string; reason?: string; resetHint?: string }): void {
    ...
    if (persisted?.source !== "builtin") {
      this.diagnostic(`[usage-limit-scheduler] ${runId}: custom or legacy script requires human approval, not arming`);
      return;
    }
  ...
  private coldStartRearm(): void {
    ...
    for (const run of runs) {
      ...
      if (run.source !== "builtin") continue;
