...
/**
 * Convert `/code-review` input into a shell-free diff command.
 *
 * A revision range is untrusted git input even when passed through execFile:
 * git still parses an argv element beginning with `-` as an option. Reject
 * that shape and terminate git's option parsing explicitly before the range.
 */
export function buildCodeReviewDiffCommand(input: string): CodeReviewDiffCommand {
  if (!input) {
    return { command: "git", args: ["diff", "HEAD"], diffSource: "git diff HEAD" };
  }
  if (/^\d+$/.test(input)) {
    return { command: "gh", args: ["pr", "diff", input], diffSource: `gh pr diff ${input}` };
  }
  if (input.includes("..")) {
    if (input.startsWith("-")) {
      throw new Error("Revision range must not start with '-'.");
    }
    return {
      command: "git",
      args: ["diff", "--end-of-options", input, "--"],
      diffSource: `git diff ${input}`,
    };
  }
  return {
    command: "git",
    args: ["diff", "HEAD", "--", input],
...
function startBackground(
  ...
  name: string,
  invocation: BuiltinWorkflowInvocation,
): void {
  try {
    const { runId } = manager.startInBackground(invocation.script, invocation.args, {
  ...
  if (!alreadyRegistered(pi, "deep-research")) {
    pi.registerCommand("deep-research", {
      ...
      async handler(args: string, ctx: ExtensionCommandContext) {
        ...
        if (!resolved) return;
        startBackground(manager, ctx, "deep-research", resolved);
      },
    });
  }
  ...
  if (!alreadyRegistered(pi, "adversarial-review")) {
    pi.registerCommand("adversarial-review", {
      ...
      async handler(args: string, ctx: ExtensionCommandContext) {
        ...
        if (!resolved) return;
        startBackground(manager, ctx, "adversarial-review", resolved);
      },
    });
  }
  ...
  if (!alreadyRegistered(pi, "code-review")) {
    pi.registerCommand("code-review", {
      ...
      async handler(args: string, ctx: ExtensionCommandContext) {
        ...
        try {
          const command = buildCodeReviewDiffCommand(input);
          diffSource = command.diffSource;
          // execFile (not exec/shell) + array args: input can't break out into a
          // shell command. Git option parsing is also terminated explicitly for
          // revision ranges by buildCodeReviewDiffCommand(). maxBuffer is raised
          // well past Node's 1MB default so a large `gh pr diff` doesn't throw
        ...
        if (!resolved) return;
        startBackground(manager, ctx, "code-review", resolved);
      },
    });
  }
  ...
  if (!alreadyRegistered(pi, "multi-perspective")) {
    pi.registerCommand("multi-perspective", {
      ...
      async handler(args: string, ctx: ExtensionCommandContext) {
        ...
        if (!resolved) return;
        startBackground(manager, ctx, "multi-perspective", resolved);
      },
    });
  }
  ...
  if (!alreadyRegistered(pi, "codebase-audit")) {
    pi.registerCommand("codebase-audit", {
      ...
      async handler(args: string, ctx: ExtensionCommandContext) {
        ...
        if (!resolved) return;
        startBackground(manager, ctx, "codebase-audit", resolved);
      },
    });
  }
}
