...
const workflowToolSchema = Type.Object({
  script: Type.Optional(
    Type.String({
      description: [
        ...
        "Custom scripts require fresh UI approval of their complete exact text; headless execution fails closed.",
        ...
        "Audited Pi disables in-script workflow(...) nesting because child code is absent from this script approval; put child phases and logic directly here.",
  ...
  name: Type.Optional(
    Type.String({
      description:
        "Run a curated built-in or saved workflow by name; pass object inputs in `args`. " +
        `Built-ins: ${BUILTIN_WORKFLOW_NAMES.join(", ")} — see the workflow-patterns skill for each one's args. ` +
        "Reserved names resolve through the curated registry. Saved scripts require fresh complete-script UI approval and fail closed headlessly. Cannot combine with resumeFromRunId.",
    }),
  ),
  args: Type.Optional(
  ...
  background: Type.Optional(
    Type.Boolean({
      description:
        "Detach the run. Default: true. Return a run ID now and post its result later. Set false only to block for an inline result.",
    }),
  ),
  ...
  resumeFromRunId: Type.Optional(
    Type.String({
      description: [
        "Resume an incomplete run with an edited `script`.",
        "Edited text requires fresh complete-script UI approval; headless resume fails closed.",
...
export function createWorkflowTool(options: WorkflowToolOptions = {}): ToolDefinition<typeof workflowToolSchema, any> {
  ...
  return defineTool({
    name: "workflow",
    label: "Workflow",
    description: "Run a JavaScript workflow that delegates to subagents via agent(), parallel(), and pipeline().",
    promptSnippet:
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      // workflow-saved.ts). Built-in names are reserved; otherwise a saved
      // workflow resolves with source="saved" so the trust gate below cannot
      // be bypassed by choosing the named path. This lets the
      ...
      let executionArgs: unknown = params.args;
      let invocationSource: WorkflowInvocationSource = params.resumeFromRunId ? "resume" : "raw";
      let invocationLabel = params.resumeFromRunId;
      let customApproval: CustomWorkflowApproval | undefined;
      let script: string;
      if (params.name) {
        ...
        invocationSource = resolved.source;
        invocationLabel = resolved.name;
        if (resolved.source === "builtin") {
          invocationTools = resolved.tools;
          invocationToolset = resolved.toolset;
          builtinAuthorization = resolved.authorization;
          executionArgs = resolved.args;
        }
      } else {
      ...
      if (params.resumeFromRunId) {
        ...
        customApproval = await requireCustomWorkflowApproval({ source: "resume", label: invocationLabel, script }, ctx);
        const resumed = await manager.resume(runId, { script, args: params.args, approval: customApproval });
        if (!resumed) {
          throw new Error(resumeFailureText(manager, runId));
        }
        return {
      ...
      if (invocationSource !== "builtin") {
        customApproval = await requireCustomWorkflowApproval(
          { source: invocationSource, label: invocationLabel, script },
          ctx,
        );
      }
      ...
      if (params.background ?? true) {
        const { runId } = manager.startInBackground(script, executionArgs, {
          ...
          source: invocationSource === "builtin" ? "builtin" : "custom",
      ...
      try {
        result = await manager.runSync(script, executionArgs, {
          maxAgents: params.maxAgents,
          concurrency: params.concurrency,
          agentRetries: params.agentRetries,
          agentTimeoutMs: params.agentTimeoutMs,
