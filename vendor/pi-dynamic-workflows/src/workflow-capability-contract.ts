...
const PRESENT_AT: PresentAtVersion = { kind: "present-at", version: WORKFLOW_PACKAGE_VERSION };
...
const capabilities: readonly CapabilityDescriptor[] = [
  runtimeGlobal("agent", {
    ...
    constraints: [
      ...
      "requested worktree isolation is mandatory; setup failure stops the agent call instead of running in the shared checkout",
    ],
  ...
  runtimeGlobal("workflow", {
    signature: "workflow(childScriptOrSavedName, childArgs?) => Promise<unknown>",
    support: CapabilitySupport.COMPATIBILITY,
    discovery: DiscoveryPlacement.NONE,
    constraints: [
      "unavailable to workflow authors in the audited Pi host; every in-script workflow call is rejected because child code is outside the approved top-level script",
      "retained only as a low-level compatibility binding for non-host embedders",
      "write child phases and agent calls directly in the one approved top-level script",
    ],
    evidence: ["tests/workflow-saved.test.ts", "tests/shared-store.test.ts", "tests/workflow-trust.test.ts"],
  }),
  ...
  runtimeGlobal("budget", {
    ...
    constraints: [
      ...
      "all agent and helper calls in the approved top-level script share the same accounting",
    ],
  }),
  ...
  toolInput("script", "script?: string", [
    ...
    "raw custom code is shown in full and requires a fresh interactive approval for the exact script before every execution",
    "custom code fails closed when an interactive confirmation UI is unavailable",
    "in-script workflow(...) child calls are unavailable in the audited Pi host",
  ]),
  toolInput("name", "name?: string", [
    "resolves one of the 5 reserved built-in patterns first, then a project/user saved workflow",
    "saved workflows cannot shadow built-in names",
    "saved workflows are custom code and require the same exact-script interactive approval as raw scripts",
    "only registry-derived one-shot provenance lets a curated built-in skip custom-code confirmation",
  ...
  toolInput("background", "background?: boolean = true", [
    ...
    "custom code is approved interactively before it is detached into the background",
  ]),
  ...
  toolInput("resumeFromRunId", "resumeFromRunId?: string", [
    ...
    "the complete edited script requires a fresh interactive approval and cannot resume headlessly",
...
export const WORKFLOW_CAPABILITY_DEFINITION: WorkflowCapabilityDefinition = {
  versions: {
    extension: WORKFLOW_PACKAGE_VERSION,
