...
- Constraint: requested worktree isolation is mandatory; setup failure stops the agent call instead of running in the shared checkout
...
- Constraint: unavailable to workflow authors in the audited Pi host; every in-script workflow call is rejected because child code is outside the approved top-level script
- Constraint: retained only as a low-level compatibility binding for non-host embedders
- Constraint: write child phases and agent calls directly in the one approved top-level script
...
- Constraint: all agent and helper calls in the approved top-level script share the same accounting
...
- Constraint: raw custom code is shown in full and requires a fresh interactive approval for the exact script before every execution
- Constraint: custom code fails closed when an interactive confirmation UI is unavailable
- Constraint: in-script workflow(...) child calls are unavailable in the audited Pi host
...
- Constraint: resolves one of the 5 reserved built-in patterns first, then a project/user saved workflow
- Constraint: saved workflows cannot shadow built-in names
- Constraint: saved workflows are custom code and require the same exact-script interactive approval as raw scripts
- Constraint: only registry-derived one-shot provenance lets a curated built-in skip custom-code confirmation
...
- Constraint: custom code is approved interactively before it is detached into the background
...
- Constraint: the complete edited script requires a fresh interactive approval and cannot resume headlessly
