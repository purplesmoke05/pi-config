...
| In-script `workflow(...)` fails | The audited host intentionally rejects every child route because its code was not included in the approved top-level script | [lifecycle](lifecycle.md#nesting-and-shared-state) |
...
6. Fix only the demonstrated authoring issue; do not rely on unsupported nesting or known out-of-scope gaps in resume accounting, checkpoint forms/timeouts, metadata validation, or budget overshoot.
