...
Resume replays only the longest unchanged prefix of journaled calls. Once one call is new, changed, or unusable, that call and all later calls execute live. Stable lexical call ordering, prompts, labels, routing options, and inputs therefore matter. Retry chains can cascade after an upstream miss.
...
The audited Pi host disables in-script `workflow(...)` nesting because saved,
computed, or argument-supplied child code is not shown in the parent approval.
Write child phases, agents, and shared-state logic directly in the approved
parent script. The low-level library retains upstream nesting behavior only for
non-host compatibility; do not author against it here.
