...
import { safeFetchText } from "../../pi-smart-fetch/core/public-http.ts";
import { createPublicUrlValidator } from "../../pi-smart-fetch/core/safe-http.ts";
import {
...
export default function extension(pi: ExtensionAPI) {
  ...
  const validatePublicUrl = createPublicUrlValidator();
  ...
  const managerOptions = {
    ...
    toolsets: {
      "web-research": () => createWebTools({ fetchText: safeFetchText, validatePublicUrl }),
    },
    ...
    isProjectTrusted: () => projectTrust?.() === true,
    // The host-facing manager is the execution boundary: custom runs require
    // exact-script approval and are never eligible for unattended auto-resume.
    enforceCustomWorkflowApproval: true,
    // Approval/built-in tokens live in generation-private WeakMaps. A manager
    // handed across /reload must verify tokens through the fresh generation
    // that issued them, so reconfigureAfterReload() swaps this set atomically.
    executionTrust: WORKFLOW_EXECUTION_TRUST,
  };
  ...
  registerAllSavedWorkflows(pi, storage, manager);
  ...
  pi.on("session_start", (_event: unknown, ctx: ExtensionContext) => {
    projectTrust = () => ctx.isProjectTrusted();
