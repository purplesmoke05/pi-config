...
const REQUIRED_RESOURCES = [
  ...
  `${SKILL_ROOT}/examples/single-script-child-jobs.js`,
...
test("one generated supported-capability table is fresh across skill, README, and website docs", () => {
  ...
  assert.doesNotMatch(generatedTable, /workflow\(childScriptOrSavedName/);
  assert.match(
...
test("generated facts cover the lifecycle constraints taught by the skill", () => {
  ...
  for (const requiredFact of [
    ...
    /unavailable to workflow authors.*every in-script workflow call is rejected/i,
...
test("generated helper facts expose exact callback, option, result, and failure contracts", () => {
  ...
  assert.match(agent?.constraints.join(" ") ?? "", /worktree isolation.*mandatory/i);
...
test("single-script child-job recipe keeps executable child logic inside the approved parent", async () => {
  const script = readFileSync(join(ROOT, SKILL_ROOT, "examples/single-script-child-jobs.js"), "utf8");
  const labels: string[] = [];
  let active = 0;
  let maxActive = 0;
  const result = await runWorkflow<{
    childResults: Array<{ id: string; status: string; result: unknown }>;
    missing: string[];
  }>(script, {
    args: { jobs: [{ id: "alpha" }, { id: "beta" }] },
    persistLogs: false,
    onAgentStart: ({ label }) => labels.push(label),
    agent: {
      async run(_prompt: string, { label }: { label?: string }) {
        ...
        return label === "prepare-child-jobs" ? { ready: true } : `result:${label}`;
      },
    },
  });

  assert.equal(maxActive, 1);
  assert.deepEqual(labels, ["prepare-child-jobs", "child:alpha", "child:beta"]);
  assert.deepEqual(Array.from(result.result.missing), ["beta"]);
  assert.deepEqual(
    Array.from(result.result.childResults, ({ id, status }) => ({ id, status })),
    [
