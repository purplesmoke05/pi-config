...
test("worktree cwd never widens an explicitly injected restricted toolset", () => {
  const webOnly = [{ name: "web_search" }, { name: "web_fetch" }] as any;
  assert.equal(
    toolsForRunCwd(webOnly, false, "/repo", "/repo/.pi/worktrees/run-1"),
    webOnly,
    "the exact restricted toolset survives worktree routing",
  );
});
