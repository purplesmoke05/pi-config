...
import type { WorkflowExecutionTrust } from "../src/workflow-manager.js";
import { WorkflowManager } from "../src/workflow-manager.js";
import type { CustomWorkflowApproval } from "../src/workflow-trust.js";
...
test(
  "reconfigureAfterReload consumes approvals through the fresh module generation",
  withTempCwd(async (cwd) => {
    const previousApproval = Object.freeze({ script: oneAgentScript });
    const freshApproval = Object.freeze({ script: oneAgentScript });
    const trustFor = (expected: CustomWorkflowApproval): WorkflowExecutionTrust => ({
      consumeCustom: (approval, script) => approval === expected && script === expected.script,
      consumeBuiltin: () => undefined,
    ...
    const manager = new WorkflowManager({
      ...
      executionTrust: trustFor(previousApproval),
    });

    await manager.runSync(oneAgentScript, undefined, { source: "custom", approval: previousApproval });
    manager.reconfigureAfterReload({
      enforceCustomWorkflowApproval: true,
      executionTrust: trustFor(freshApproval),
    });

    await assert.rejects(
      manager.runSync(oneAgentScript, undefined, { source: "custom", approval: previousApproval }),
      /fresh interactive approval/i,
    );
    const result = await manager.runSync(oneAgentScript, undefined, {
      source: "custom",
