...
describe("installWorkflowKeywordArming", () => {
  ...
  it("registers /workflows-trigger and toggles the keyword trigger", async () => {
    ...
    assert.equal(state.keywordTriggerEnabled, true, "explicit persisted opt-in should load as on");
  ...
  it("keeps keyword triggering disabled by default when the setting is absent or loading fails", async () => {
    const mod = await load();
    const stores = [
      { load: () => ({}), save: () => {} },
      {
