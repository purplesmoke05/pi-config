...
describe("config", () => {
  it("exports expected constants", async () => {
    ...
    assert.equal("WORKFLOW_RUNS_DIR" in c, false);
    assert.equal("WORKFLOW_SAVED_DIR" in c, false);
...
describe("logger", () => {
  ...
  it("persists log directories and files with owner-only permissions, including existing paths", async () => {
    const cwd = mkdtempSync(join(tmpdir(), "pi-dw-logger-cwd-"));
    const fakeHome = mkdtempSync(join(tmpdir(), "pi-dw-logger-home-"));
    try {
      await withFakeHomeAsync(fakeHome, async () => {
        const { createWorkflowLogger } = await loadLogger();
        const log = createWorkflowLogger({ cwd, runId: "private-log" });
        log.log("private entry");

        const runsDir = workflowProjectPaths(cwd).runsDir;
        const logPath = join(runsDir, "private-log.log");
        assert.equal(statSync(runsDir).mode & 0o777, 0o700);
        assert.equal(statSync(logPath).mode & 0o777, 0o600);

        chmodSync(runsDir, 0o777);
        chmodSync(logPath, 0o666);
        assert.equal(log.persist(), logPath);
        assert.equal(statSync(runsDir).mode & 0o777, 0o700);
        assert.equal(statSync(logPath).mode & 0o777, 0o600);
      });
    } finally {
  ...
  it("surfaces log persistence setup failures", async () => {
    const cwd = mkdtempSync(join(tmpdir(), "pi-dw-logger-cwd-"));
    const fakeHome = mkdtempSync(join(tmpdir(), "pi-dw-logger-home-"));
    try {
      writeFileSync(join(fakeHome, ".pi"), "not a directory", "utf8");
      await withFakeHomeAsync(fakeHome, async () => {
        const { createWorkflowLogger } = await loadLogger();
        assert.throws(() => createWorkflowLogger({ cwd, runId: "setup-failure" }), /ENOTDIR/);
      });
    } finally {
      rmSync(cwd, { recursive: true, force: true });
      rmSync(fakeHome, { recursive: true, force: true });
  ...
  it("surfaces append failures instead of silently dropping persisted logs", async () => {
    const cwd = mkdtempSync(join(tmpdir(), "pi-dw-logger-cwd-"));
    const fakeHome = mkdtempSync(join(tmpdir(), "pi-dw-logger-home-"));
    try {
      await withFakeHomeAsync(fakeHome, async () => {
        const { createWorkflowLogger } = await loadLogger();
        const log = createWorkflowLogger({ cwd, runId: "append-failure" });
        mkdirSync(join(workflowProjectPaths(cwd).runsDir, "append-failure.log"));
        assert.throws(() => log.log("must fail"), /EISDIR/);
      });
    } finally {
      rmSync(cwd, { recursive: true, force: true });
      rmSync(fakeHome, { recursive: true, force: true });
  ...
  it("rejects path-unsafe log run IDs before touching persistence", async () => {
    const { createWorkflowLogger } = await loadLogger();
    assert.throws(() => createWorkflowLogger({ persist: false, runId: "../../settings" }), /Run ID must be/);
  });
});
