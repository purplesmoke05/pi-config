...
async function execute(
  ...
  ctx: unknown = { hasUI: true, ui: { confirm: async () => true } },
) {
  const tool = createWorkflowControlTool({ manager });
  return (tool.execute as any)("control-call", params, undefined, undefined, ctx);
}
...
test("workflow_control exposes only list, status, pause, resume, and stop in a strict schema", () => {
  ...
  assert.match(tool.description, /custom and legacy scripts require fresh complete-script interactive approval/i);
  assert.match(
    JSON.stringify(tool.parameters),
    /custom and legacy code requires fresh complete-script interactive approval/i,
  );
  assert.match(JSON.stringify(tool.parameters), /re-validates curated built-in provenance/i);
...
test("workflow_control resume fails closed headlessly for custom code but not for built-ins", async () => {
  const customFixture = fakeManager([run("paused", "custom-run")]);
  const customResult = await execute(
    customFixture.manager,
    { action: "resume", runId: "custom-run" },
    { hasUI: false, ui: {} },
  );
  assert.match(text(customResult), /result=error/);
  assert.match(text(customResult), /headless|confirmation UI/i);
  assert.equal(customFixture.calls.length, 0, "custom manager.resume must not be reached");

  const builtin = run("paused", "builtin-run");
  builtin.source = "builtin";
  const builtinFixture = fakeManager([builtin]);
  const builtinResult = await execute(
    builtinFixture.manager,
    { action: "resume", runId: "builtin-run" },
    { hasUI: false, ui: {} },
  );
  assert.match(text(builtinResult), /result=resumed/);
});
