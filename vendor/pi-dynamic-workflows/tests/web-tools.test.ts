...
test("createWebFetchTool accepts maxChars parameter", () => {
  const toolSmall = createWebFetchTool(runtime, 100);
  const toolLarge = createWebFetchTool(runtime, 10000);
...
test("createWebTools returns both tools in correct order", () => {
  const tools = createWebTools(runtime);
