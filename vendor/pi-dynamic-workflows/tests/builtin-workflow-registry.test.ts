...
test(
  ...
  withTempCwd((cwd) => {
    ...
    assert.equal(invocation.tools, undefined, "the host resolves the audited toolset at execution time");
  }),
);
...
test(
  "resolveWorkflowInvocation reserves built-in names so saved workflows cannot shadow them",
  withTempCwd((cwd) => {
    const storage = createWorkflowStorage(cwd);
...
test(
  "resolveWorkflowInvocation marks non-reserved saved workflows as custom source",
  withTempCwd((cwd) => {
    const storage = createWorkflowStorage(cwd);
    const customScript = "export const meta = { name: 'custom', description: 'custom' }\nreturn 1";
    storage.save({ name: "custom", description: "custom", script: customScript });

    const resolved = resolveWorkflowInvocation("custom", {}, { storage, cwd });
    assert.ok(resolved);
