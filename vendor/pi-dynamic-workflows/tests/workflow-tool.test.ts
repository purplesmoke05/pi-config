...
function approvalContext(approved = true) {
...
test("createWorkflowTool description states its delegation capability", () => {
  ...
  assert.match(description, /JavaScript workflow.*delegates to subagents/i);
  assert.match(description, /agent\(\).*parallel\(\).*pipeline\(\)/i);
...
test("createWorkflowTool keeps script syntax in the parameter schema", () => {
  ...
  assert.match(description, /fresh UI approval.*complete exact text/i);
  assert.match(description, /headless execution fails closed/i);
  ...
  assert.match(description, /disables in-script workflow\(\.\.\.\) nesting.*child code.*script approval/i);
  assert.match(description, /put child phases and logic directly here/i);
  assert.match(
    description,
...
test("workflow name and edited-resume schemas expose the custom-code approval boundary", () => {
  const tool = createWorkflowTool();
  const name = parameterDescription(tool, "name");
  const resume = parameterDescription(tool, "resumeFromRunId");

  assert.match(name, /saved scripts require fresh complete-script UI approval/i);
  assert.match(name, /reserved names resolve through the curated registry/i);
  assert.match(resume, /edited text requires fresh complete-script UI approval/i);
  assert.match(resume, /headless resume fails closed/i);
});
...
test("createWorkflowTool keeps background behavior in the parameter schema", () => {
  ...
  assert.match(description, /post its result later/i);
  assert.match(description, /false only to block for an inline result/i);
...
test(
  ...
  withToolTempCwd(async (cwd) => {
    const manager = new WorkflowManager({ cwd, agent: toolFakeAgent() });
    const tool = createWorkflowTool({ cwd, manager });
    const { ctx, confirmations } = approvalContext();
    const res = await tool.execute("t4", { script: resumeToolScript }, undefined, undefined, ctx);
    assert.equal(confirmations.length, 1);
    assert.match(confirmations[0].message, /arbitrary Node\.js code/i);
    assert.ok(
      confirmations[0].message.includes(renderWorkflowScriptForApproval(resumeToolScript)),
      "confirmation shows the complete quoted raw script",
    );
...
return { a, b }`;
    ...
    const { ctx, confirmations } = approvalContext();
    const res = await tool.execute("t5", { script: v2, resumeFromRunId: runId }, undefined, undefined, ctx);
    assert.equal(confirmations.length, 1);
    assert.ok(
      confirmations[0].message.includes(renderWorkflowScriptForApproval(v2)),
      "resume confirmation shows the complete quoted edited script",
    );
...
test(
  "workflow tool: a built-in name cannot be shadowed by a saved workflow",
  withToolTempCwd(async (cwd) => {
    ...
    const managed = manager.getRun(runId);
    assert.equal(managed?.snapshot.name, "deep_research", "the curated built-in should win");
    assert.equal(managed?.toolset, "web-research", "the built-in keeps its audited exec context");
    await new Promise((r) => setTimeout(r, 50));
  }),
);

test(
  "workflow tool: a non-reserved saved workflow requires approval of its complete script",
  withToolTempCwd(async (cwd) => {
    const storage = createWorkflowStorage(cwd);
    const script = "export const meta = { name: 'custom_saved', description: 'custom' }\nreturn 1";
    storage.save({ name: "custom-saved", description: "custom", script });
    const manager = new WorkflowManager({ cwd, agent: toolFakeAgent("ok") });
    const tool = createWorkflowTool({ cwd, manager, storage });
    const { ctx, confirmations } = approvalContext();

    const res = await tool.execute("saved", { name: "custom-saved" }, undefined, undefined, ctx);

    assert.ok((res.details as { runId?: string }).runId);
    assert.equal(confirmations.length, 1);
    assert.match(confirmations[0].message, /saved workflow "custom-saved"/i);
    assert.ok(confirmations[0].message.includes(renderWorkflowScriptForApproval(script)));
  }),
);

test(
  "workflow tool: custom scripts fail closed in headless mode and on rejection",
  withToolTempCwd(async (cwd) => {
    const manager = new WorkflowManager({ cwd, agent: toolFakeAgent("ok") });
    const tool = createWorkflowTool({ cwd, manager });

    await assert.rejects(
      () => tool.execute("headless", { script: resumeToolScript }, undefined, undefined, undefined),
      /headless|confirmation UI/i,
    );
    const rejected = approvalContext(false);
    await assert.rejects(
      () => tool.execute("rejected", { script: resumeToolScript }, undefined, undefined, rejected.ctx),
      /not approved/i,
    );
    assert.equal(manager.listRuns().length, 0, "neither denied path may create or execute a run");
  }),
);
