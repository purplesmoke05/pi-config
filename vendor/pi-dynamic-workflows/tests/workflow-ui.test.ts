...
test("restarting a built-in re-derives script, execution context, and one-shot provenance from the registry", async () => {
  const { ui, notifications, getComponent } = fakeUiCapturingComponent();
  const args = { question: "verify restart provenance" };
  const expected = findBuiltinWorkflow("deep-research")?.resolve(process.cwd(), args);
  assert.ok(expected);
  let started: { script: string; args: unknown; exec: Parameters<WorkflowManager["startInBackground"]>[2] } | undefined;
  const fakeManager = {
    on: () => {},
    off: () => {},
    listRuns: () =>
      [
        {
          ...
          agents: [],
          logs: [],
          script: "tampered persisted script",
          args,
    ...
    startInBackground: (
      ...
      exec: Parameters<WorkflowManager["startInBackground"]>[2],
    ) => {
      started = { script, args: executionArgs, exec };
      return { runId: "restarted-builtin", promise: Promise.resolve({}) };
    },
  } as unknown as WorkflowManager;
  ...
  assert.equal(started?.script, expected.script);
  assert.deepEqual(started?.args, expected.args);
  assert.equal(started?.exec?.source, "builtin");
  assert.equal(started?.exec?.toolset, "web-research");
  assert.ok(started?.exec?.builtinAuthorization);
  assert.equal(notifications[0]?.type, "info");
  assert.match(notifications[0]?.message ?? "", /Restarted deep_research as restarted-builtin/);
});
