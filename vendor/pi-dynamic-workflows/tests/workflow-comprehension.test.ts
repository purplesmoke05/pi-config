...
const alpha = await agent("run child alpha", { label: "child:alpha" })
const beta = await agent("run child beta", { label: "child:beta" })
return { alpha, beta }`,
    ],
    [
      "full-review",
...
test("edit evidence identifies preparation by timeline and requires returned inline child results", async () => {
...
await agent("run child alpha", { label: "child:alpha" })
await agent("run child beta", { label: "child:beta" })
...
test("edit evidence requires sequential inline child agents after preparation", async () => {
  const scenario = selectComprehensionScenarios("full").find(({ id }) => id === "full-edit");
  assert.ok(scenario);
...
return { alpha, beta }`;
  ...
  assert.equal(evidence.passed, false);
  assert.equal(evidence.runtime.assertions.find(({ name }) => name === "child-jobs:sequential")?.passed, false);
});
