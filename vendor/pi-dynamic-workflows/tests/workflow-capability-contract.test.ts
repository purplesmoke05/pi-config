...
test("nested workflow binding is compatibility-only and undiscoverable in the audited host contract", () => {
  const nestedWorkflow = WORKFLOW_CAPABILITY_DEFINITION.capabilities.find(
    ({ id }) => id === "workflow.runtime.workflow",
  );

  assert.equal(nestedWorkflow?.support, CapabilitySupport.COMPATIBILITY);
  assert.equal(nestedWorkflow?.discovery, DiscoveryPlacement.NONE);
  assert.match(nestedWorkflow?.constraints.join(" ") ?? "", /every in-script workflow call is rejected/i);
});
