...
describe("loadAgentRegistry", () => {
  ...
  it("does not load project definitions unless project trust is explicit", () => {
    const root = mkdtempSync(join(tmpdir(), "agents-untrusted-"));
    ...
    assert.equal(reg.get("reviewer")?.model, "user/model");
    assert.equal(reg.get("reviewer")?.source, "user");
    rmSync(root, { recursive: true, force: true });
  });

  it("rejects a trusted project definition that escapes through a file symlink", () => {
    const root = mkdtempSync(join(tmpdir(), "agents-symlink-file-"));
    const projectDir = join(root, ".pi", "agents");
    const outside = mkdtempSync(join(tmpdir(), "agents-outside-"));
    mkdirSync(projectDir, { recursive: true });
    writeDef(outside, "escaped.md", "---\nname: escaped\n---\noutside body");
    symlinkSync(join(outside, "escaped.md"), join(projectDir, "escaped.md"));
    ...
    assert.equal(reg.has("escaped"), false);
    rmSync(root, { recursive: true, force: true });
    rmSync(outside, { recursive: true, force: true });
  });

  it("rejects a trusted project agent directory that is itself a symlink", () => {
    const root = mkdtempSync(join(tmpdir(), "agents-symlink-dir-"));
    const outside = mkdtempSync(join(tmpdir(), "agents-outside-dir-"));
    const projectParent = join(root, ".pi");
    const projectDir = join(projectParent, "agents");
