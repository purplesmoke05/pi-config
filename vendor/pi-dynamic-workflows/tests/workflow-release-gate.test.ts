...
test("contributor docs explain the protected workflow-authoring guidance gate", () => {
  const contributing = readFileSync(new URL("../UPSTREAM-CONTRIBUTING.md", import.meta.url), "utf8");
...
test("the vendor omits upstream repository instructions and records why", () => {
  assert.equal(existsSync(new URL("../AGENTS.md", import.meta.url)), false);
  const patches = readFileSync(new URL("../SOURCE-PATCHES.md", import.meta.url), "utf8");
  assert.match(patches, /Did not vendor upstream `AGENTS\.md`/);
  assert.match(patches, /affect the auditing agent rather than the\s+extension runtime/);
});
