...
describe("workflow paths", () => {
  ...
  it("keeps project storage under workflow home", () => {
    withIsolatedHome((home, cwd) => {
      ...
      assert.equal(Object.hasOwn(paths, "legacyRunsDir"), false);
      assert.equal(Object.hasOwn(paths, "legacySavedDir"), false);
      assert.equal(paths.rootDir.startsWith(cwd), false, "repository-local state is not part of the layout");
    });
  });
});
