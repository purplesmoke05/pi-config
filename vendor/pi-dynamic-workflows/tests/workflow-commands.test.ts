...
function harness(
  ...
  uiOptions: { hasUI?: boolean; confirmResult?: boolean } = {},
) {
  ...
  const confirmed: Array<{ title: string; message: string }> = [];
  const calls: string[] = [];
  ...
  return { run, printed, sent, notified, confirmed, calls, activeTools };
}
...
test("/workflows resume requires approval of a custom persisted script", async () => {
  const script = "export const meta = { name: 'custom', description: 'custom' }; return 1";
  let approvalScript: string | undefined;
  const h = harness(
    {
      listRuns: () => [
        {
      ...
      resume: async (_id: string, opts: { approval?: { script?: string } }) => {
        approvalScript = opts.approval?.script;
        return true;
      },
    },
    {},
  ...
  await h.run("resume custom-run");

  assert.equal(h.confirmed.length, 1);
  assert.ok(h.confirmed[0].message.includes(renderWorkflowScriptForApproval(script)));
  assert.match(h.confirmed[0].message, /arbitrary Node\.js code/i);
  assert.equal(approvalScript, script, "the manager receives approval bound to the exact script");
});
