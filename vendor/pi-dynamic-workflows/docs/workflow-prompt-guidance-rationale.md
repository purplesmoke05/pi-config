...
### Disable in-script nesting at the audited host boundary

The upstream 3.5.0 guidance made `workflow(savedName, childArgs)` discoverable
from `script.description`. This audited build replaces that sentence with:
...
The parent confirmation displays only the parent script. A saved, computed, or
argument-supplied child can therefore introduce executable code that the user
never saw. The hardened `WorkflowManager` disables every nested `workflow(...)`
call instead of trying to infer which dynamic child values were visible. The
low-level runtime retains upstream compatibility for non-host consumers, but Pi
execution always goes through the hardened manager.

#### Evidence

`tests/workflow-trust.test.ts` covers saved and argument-supplied child scripts
under custom and built-in parents. Runtime compatibility tests continue to cover
the low-level nesting implementation separately.

#### Confidence

Confidence is high: a single fail-closed host rule is auditable, while partial
saved-name checks leave computed child scripts outside the approval boundary.
