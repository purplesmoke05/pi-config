...
import { loadPiSmartFetchSettings } from "./settings.ts";

const toolDescription = [
  "Fetch a public HTTP/HTTPS URL with Node's standard http/https transport and extract clean, readable content.",
  "Uses pinned public DNS, manual redirect checks, and Defuddle for article extraction.",
...
const batchToolDescription = [
  "Fetch up to 10 public HTTP/HTTPS URLs with Node's standard http/https transport and readable extraction.",
...
function renderBatchProgressText(
  ...
  const rows = snapshot.items.map((item, index) => {
    ...
    const url = theme.fg(
      "accent",
      truncateMiddle(sanitizeSingleLineText(item.url), urlWidth),
    );
    ...
    return `${baseRow}\n  ${theme.fg("error", `error: ${sanitizeSingleLineText(item.error)}`)}`;
  });
...
function renderUserMetadataLine(
  ...
  return (
    ...
    theme.fg("syntaxString", sanitizeSingleLineText(value))
  );
}
...
function buildWebFetchCollapsedPreview(details: WebFetchRenderDetails): {
  ...
  const contentLines = stripTerminalControlCharacters(
    details.fetchResult?.content ?? "",
  )
    .split("\n")
    .filter(
      (line, index, lines) =>
...
function createWebFetchResultComponent(
    ...
    const fileLines = [
      ...
      ...(fetchResult.mimeType
        ? [
            theme.fg(
              "muted",
              `Mime type: ${sanitizeSingleLineText(fetchResult.mimeType)}`,
            ),
          ]
        : []),
      theme.fg(
        "muted",
        `File path: ${sanitizeSingleLineText(fetchResult.filePath)}`,
      ),
    ];
  ...
  const content = expanded
    ? stripTerminalControlCharacters(fetchResult.content)
    : previewContent;
...
function renderSingleFetchProgressText(
  ...
  const renderedUrl = theme.fg(
    "accent",
    truncateMiddle(sanitizeSingleLineText(url), urlWidth),
  );
...
export default function piSmartFetchExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "web_fetch(url, headers?, maxChars?, timeoutMs?, format?, removeImages?, includeReplies?, verbose?): fetch readable web content over a pinned Node HTTP/HTTPS transport with full agent metadata and a compact pi preview",
      ...
      let latestDetails: WebFetchRenderDetails = {
        ...
        url:
          typeof params.url === "string"
            ? sanitizeSingleLineText(params.url)
            : undefined,
        spinnerTick,
      };
      ...
      const emitProgress = (details: WebFetchRenderDetails) => {
        ...
        onUpdate?.({
          content: [
            {
              type: "text",
              text: `Fetching ${sanitizeSingleLineText(
                details.url ??
                  (typeof params.url === "string" ? params.url : "URL"),
              )}...`,
            },
          ],
          details,
      ...
      try {
        ...
        return {
          ...
          details: {
            ...
            progress: 1,
            phase: "done",
            url: sanitizeSingleLineText(result.finalUrl || result.url),
            spinnerTick,
          } satisfies WebFetchRenderDetails,
        };
      } catch (error) {
        ...
        const requestedUrl = sanitizeSingleLineText(
          typeof params.url === "string" ? params.url : "URL",
        );
        const errorText = `Error: Unexpected web_fetch failure for ${requestedUrl}.\n\n${message}`;
        return {
          content: [{ type: "text", text: errorText }],
          details: {
            error: true,
    ...
    renderCall(args: Record<string, unknown>, theme) {
      const url = sanitizeSingleLineText(
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "batch_web_fetch(requests, verbose?): fetch up to 10 URLs concurrently (maximum concurrency 4) with full agent metadata and per-item progress in the pi TUI",
    parameters: Type.Object({
