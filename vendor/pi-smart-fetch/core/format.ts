...
function buildHeader(
  ...
  return parts
    ...
    .map(
      ([label, value]) =>
        `> ${label}: ${sanitizeSingleLineText(value ?? "")}`,
    )
    .join("\n");
}
...
export function buildUserFacingFetchErrorSummary(error: FetchError): string {
  if (error.code === "http_error" && error.statusCode) {
    return `Server responded with ${error.statusCode}${error.statusText ? ` ${sanitizeSingleLineText(error.statusText)}` : ""}`;
  }
  ...
  switch (error.code) {
    ...
    default:
      return sanitizeSingleLineText(error.error);
  }
}

export function buildFetchErrorResponseText(error: FetchError): string {
  const lines = [`Error: ${sanitizeSingleLineText(error.error)}`];
  ...
  if (error.code === "timeout") {
    ...
    if (error.statusCode === 429) {
      lines.push(
        "",
        "The server rate-limited this request. Retrying later may help.",
      );
    } else if (error.statusCode === 401 || error.statusCode === 403) {
      lines.push(
        "",
        "The server rejected this request. The target may require authentication or a different request context.",
      );
...
/** Prevent fetched content from writing terminal control sequences in Pi's Text/Markdown UI. */
export function stripTerminalControlCharacters(content: string): string {
...
export function sanitizeSingleLineText(value: string | number): string {
  return stripTerminalControlCharacters(String(value))
    .replace(/[\r\n\t\u2028\u2029]+/g, " ")
    .trim();
}

export function truncateContent(content: string, maxChars: number): string {
  const safeContent = stripTerminalControlCharacters(content);
  if (safeContent.length <= maxChars) return safeContent;
  return `${safeContent.slice(0, maxChars)}\n\n[... truncated]`;
}
...
export function buildFetchResponseText(
  ...
  if (isFileFetchResult(result)) {
    return stripTerminalControlCharacters(header);
  }
...
function buildBatchItemHeading(
  ...
  return `## [${ordinal}/${total}] ${sanitizeSingleLineText(url)}`;
}
...
function buildBatchItemText(
  ...
  if (item.status === "error") {
    const errorText = stripTerminalControlCharacters(
...
export function buildBatchFetchResponseText(
  ...
  return stripTerminalControlCharacters(
    [summary, ...items].filter(Boolean).join("\n\n"),
  );
}
