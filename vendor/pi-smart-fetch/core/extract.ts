/**
 * Core extraction pipeline: pinned Node HTTP → parse → Defuddle extract.
...
import { getLatestChromeProfile as getLatestChromeProfileFrom } from "./profiles.ts";
import {
  MAX_FILE_BYTES,
  MAX_TEXT_BYTES,
  PublicUrlError,
...
import { deburr, extensionForMimeType } from "./text-utils.ts";
import type {
  FetchDependencies,
  FetchError,
  FetchErrorPhase,
...
function resolveTimeoutMs(opts: FetchOptions): number {
  return typeof opts.timeoutMs === "number" && Number.isFinite(opts.timeoutMs) && opts.timeoutMs > 0
    ? Math.min(MAX_TIMEOUT_MS, Math.floor(opts.timeoutMs))
    : DEFAULT_TIMEOUT_MS;
}

function deadlineExceededError(timeoutMs: number): Error {
...
function assertOperationActive(
  ...
  if (Date.now() >= deadlineAt) throw deadlineExceededError(timeoutMs);
}
...
async function waitForOperation<T>(
  ...
  assertOperationActive(signal, deadlineAt, timeoutMs);
...
function optionsForHop(
  opts: FetchOptions,
  currentUrl: string,
  nextUrl: string,
  credentialsAlreadyStripped: boolean,
): FetchOptions {
  const original = new URL(opts.url);
  const current = new URL(currentUrl);
  const next = new URL(nextUrl, currentUrl);
  if (
    !credentialsAlreadyStripped &&
    original.origin === current.origin &&
    current.origin === next.origin
  ) {
    return { ...opts, url: next.toString() };
  }

  return {
    ...opts,
    url: next.toString(),
    headers: stripCredentialHeaders(opts.headers ?? {}),
  };
}
...
function resolveExtensionFromMimeType(contentType: string): string {
  const extension = extensionForMimeType(normalizeContentType(contentType));
  return (
...
export async function streamResponseToFile(
  ...
  if (response.body) {
    ...
    try {
      ...
      while (true) {
        ...
        if (value) {
          if (fileSize + value.byteLength > maxBytes) {
      ...
      if (
        ...
        // No response bytes have been read yet. Keep the body intact so the
        // caller can retry with an exclusive alternate filename.
        throw error;
      }
      try {
        await reader.cancel(
  ...
  if (response.readable) {
    ...
    source.on("data", (chunk: string | ArrayBufferView) => {
      ...
      if (fileSize > maxBytes) {
        (source as NodeJS.ReadableStream & { destroy(error?: Error): void }).destroy(
          new ResponseBodyTooLargeError(maxBytes, fileSize),
        );
      }
    });
    try {
  ...
  if (fileSize > maxBytes) {
    throw new ResponseBodyTooLargeError(maxBytes, fileSize);
  }
  try {
...
function isAbortError(error: unknown): boolean {
  return (
    ...
    /\baborted\b/i.test(error instanceof Error ? error.message : String(error))
  );
}
...
async function buildFileResult(
  ...
  // Downloads intentionally do not honour project settings or a caller
  // supplied tempDir.  Keeping them under the agent's private cache prevents
  // a project from redirecting fetched files into an arbitrary writable path.
  const tempDir = join(getAgentDir(), "cache", "pi-smart-fetch", "downloads");
  ...
  while (attempt <= 100) {
    try {
      const fileSize = await streamResponseToFile(
        response,
        filePath,
        MAX_FILE_BYTES,
      );
...
export function createDefuddleFetch(
  ...
  async function fetchWithClientRedirects(
    ...
    // Browser/OS fingerprint options from upstream are intentionally ignored:
    // this vendored implementation uses Node's standard http/https transport.
    const browser = "node";
    ...
    const maxChars =
      typeof opts.maxChars === "number" && Number.isFinite(opts.maxChars) && opts.maxChars > 0
        ? Math.min(MAX_RETURN_CHARS, Math.floor(opts.maxChars))
        : DEFAULT_MAX_CHARS;
    ...
    try {
      assertOperationActive(opts.signal, deadlineAt, timeoutMs);
      emitProgress(hooks, {
        status: "connecting",
        progress: 0,
        phase: "fetch_start",
      ...
      if (!response.ok) {
        if (response.body && !response.body.locked) {
          const reader = response.body.getReader();
          await reader.cancel("discarding non-success response").catch(() => undefined);
          reader.releaseLock();
        }
        return {
      ...
      if (shouldDownloadToFile) {
        ...
        if (
          ...
          errorContext.contentLength > MAX_FILE_BYTES
        ) {
          throw new ResponseBodyTooLargeError(
            MAX_FILE_BYTES,
            errorContext.contentLength,
      ...
      if (
        errorContext.contentLength !== undefined &&
        errorContext.contentLength > MAX_TEXT_BYTES
      ) {
        throw new ResponseBodyTooLargeError(
          MAX_TEXT_BYTES,
          errorContext.contentLength,
      ...
      if (
        ...
        assertHtmlWithinComplexityLimits(rawBody);
      }
      ...
      if (clientSideRedirect) {
        ...
        return fetchWithClientRedirects(
          optionsForHop(
            ...
            finalUrl,
            clientSideRedirect,
            response.credentialsStripped === true,
          ),
          hooks,
      ...
      if (format === "raw") {
        // Raw mode skips extraction. The JS-disabled shell is still a useful
        // local signal for deleted/protected X posts, and requires no network.
        const isXUrl = /^https?:\/\/(www\.)?(x\.com|twitter\.com)\//i.test(
          opts.url,
        );
        if (isXUrl && isTwitterJsDisabledPage(parseLinkedomHTML(rawBody, finalUrl), opts.url)) {
          return {
        ...
        const effectiveContent =
          opts.maxChars !== undefined
            ...
            : stripTerminalControlCharacters(rawBody);
      ...
      const extracted = await waitForOperation(
        () =>
          dependencies.defuddleHtml
            ? dependencies.defuddleHtml(
                rawBody,
                finalUrl,
                defuddleOptions,
      ...
      // Detect X/Twitter deleted/protected tweets from the local JS-disabled
      // shell. Defuddle async extractors are disabled and never make an
      // oEmbed request here.
      const isXUrl = /^https?:\/\/(www\.)?(x\.com|twitter\.com)\//i.test(
        opts.url,
      );
      if (isXUrl) {
        ...
        if (hasJsDisabledShell && !extracted.content) {
          return {
            error: `Server returned HTTP 404 Not Found for ${opts.url}.`,
            code: "http_error",
            phase: "loading",
