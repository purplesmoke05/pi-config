...
For a new conversation, the first Copilot provider request shows a temporary widget with detected file tags and paths. It separates automatic context (`AGENTS.md`, `CLAUDE.md`, and GitHub Copilot instruction files) from tagged prompt files. Pi flattens `@file` inputs into `<file name="…">` text and exposes no structured origin metadata to extensions, so the prompt-file group is heuristic tag detection rather than a provenance guarantee; manually authored identical markup and an image marker whose bytes were omitted are indistinguishable. Displayed paths escape terminal control characters, and the widget reserves space for both groups within Pi's ten-line limit. The widget remains through the first agent run and is cleared when the next user message starts, including queued steer/follow-up messages. Skill bodies are not listed because Pi keeps them on-demand; a zero-file widget also states that the system prompt and tool schemas are still included. Resumed conversations that already contain an assistant response do not show the widget again.
