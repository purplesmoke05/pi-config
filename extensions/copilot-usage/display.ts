...
/** Detailed monthly quota lines for the /copilot-usage report. */
export function quotaReportLines(quota: CopilotQuota | null | undefined): string[] {
	if (!quota) return ["  not requested · run /copilot-usage"];
	const premium = quota.categories.premium_interactions;
...
/** Detailed per-remaining-business-day budget line for the /copilot-usage report. */
...
/** Compact per-day budget for the credit status line, or null when not computable. */
